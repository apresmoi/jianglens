---
id: glossary:pastoral-conquest
title: Pastoral Conquest
description: Jiang Lens glossary entry for pastoral conquest, with dated source refs, episode links, aliases, and related lens pages.
kind: glossary
aliases:
  - steppe conquest
  - Yamnaya conquest
  - nomadic pastoralism
  - pastoralist war machine
related_lens:
  - civilization-as-inner-order
  - the-borderland-engine
  - game-theory
---

<!-- Generated by ops/scripts/compile-content.mjs. Do not edit by hand. Source: content/lens/glossary/pastoral-conquest.md. -->

# Pastoral Conquest

**Aliases:** steppe conquest, Yamnaya conquest, nomadic pastoralism, pastoralist war machine.

> **Fast answer:** Pastoral conquest names Jiang's civilizational pattern in which mobile herd societies convert grassland, cattle, horses, wagons, property, patriarchy, and warrior status into a conquest system. The Yamnaya and Mongol cases show how scarcity and mobility can overpower settled orders.

## What Jiang Means

The mechanism begins with ecology. Humans cannot eat grass, but animals can. Herds convert grass into mobile wealth, and mobile wealth requires defense, inheritance, masculinity, and fighting capacity. Horses and wagons extend the system into movement and reach.

Pastoral conquest is not a racial claim. Jiang repeatedly treats it as culture and material order: a way of life that produces values, constraints, and military advantages. It belongs beside civilization-as-inner-order because pastoralism creates an inner order different from farming civilization.

## Where Jiang Says It

| Source | Timestamp / ref | What to inspect | Why it matters |
|---|---|---|---|
| 2024-09-12, [How the Yamnaya Made War, Money, and the West](/episodes/predictive-history-j4htfjwl5d8/) | `video:predictive-history-j4htfjwl5d8@transcript:v1#seg-0005`, `#seg-0011`, `#seg-0016` | Grass, cattle, pastoral order | Core material mechanism. |
| 2024-09-10, [The Civilization That Chose Not To Make War](/episodes/predictive-history-rat-zudjhrm/) | `video:predictive-history-rat-zudjhrm@transcript:v1#seg-0014`, `#seg-0015`, `#seg-0021` | Diffusion versus conquest; Kurgan graves | Yamnaya conquest evidence. |
| 2025-04-10, [The World Shatterer](/episodes/predictive-history-mpqzapnjykm/) | `video:predictive-history-mpqzapnjykm@transcript:v1#seg-0001`, `#seg-0008`, `#seg-0046` | Mongols as steppe iteration | Later pastoral empire case. |

## How To Use This Term

Use this term when Jiang is explaining conquest through pastoral ecology, mobility, herds, horses, property, and warrior culture.

Do not use it as a generic label for any invasion.

## Related Lens Pages

- [Civilization As Inner Order](/lens/civilization-as-inner-order/)
- [The Borderland Engine](/lens/the-borderland-engine/)
- [Game Theory](/lens/game-theory/)
