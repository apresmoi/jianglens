...
- The maintainer wants autonomous workers that improve through practice, but this worker should not edit `.codex/skills/**` while it is the first participant. Propose skill/process changes under the repo checkout path `agentic-org/agents/virgil/proposals/` or in PR notes.
...
- Episode stops at website-visible episode publication. Corpus impact and lens/canon mutation are separate downstream jobs.
...
- For worker self-diagnosis or self-improvement tasks, edit only the repo checkout path `agentic-org/agents/virgil/**` unless the maintainer explicitly expands scope.
