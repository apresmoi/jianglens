...
const contentGlossaryRoot = path.join(contentRoot, 'lens/glossary');
const websiteGlossaryIndexPath = path.join(websiteDocsRoot, 'glossary.md');
const websiteGlossaryDocsRoot = path.join(websiteDocsRoot, 'glossary');
...
function parseFrontmatterList(text, key) {
  const raw = frontmatterBody(text);
  if (!raw) return [];
...
function insertGeneratedComment(text, sourceRelPath) {
  const comment = `<!-- ${generatedHeader} Source: ${sourceRelPath}. -->`;
  if (!text.startsWith('---\n')) return `${comment}\n\n${text.trim()}\n`;
  ...
  const body = text.slice(frontmatterEnd).replace(/^\s+/, '');

  return `${frontmatter}\n\n${comment}\n\n${body.trim()}\n`;
}

async function writeGlossaryDocs() {
  await rm(websiteGlossaryDocsRoot, { recursive: true, force: true });
  await rm(websiteGlossaryIndexPath, { force: true });

  if (!existsSync(contentGlossaryRoot)) return [];

  const entries = (await readdir(contentGlossaryRoot, { withFileTypes: true }))
    .filter((entry) => entry.isFile() && entry.name.endsWith('.md') && !['README.md', 'index.md'].includes(entry.name))
    .map((entry) => ({
      slug: entry.name.replace(/\.md$/, ''),
      inputPath: path.join(contentGlossaryRoot, entry.name),
      outputPath: path.join(websiteGlossaryDocsRoot, entry.name),
    }))
    .sort((a, b) => a.slug.localeCompare(b.slug));

  await mkdir(websiteGlossaryDocsRoot, { recursive: true });

  const indexEntries = [];
  for (const entry of entries) {
    const text = await readFile(entry.inputPath, 'utf8');
    const metadata = parseFrontmatter(text);
    const relPath = path.relative(repoRoot, entry.inputPath);
    await writeFile(entry.outputPath, insertGeneratedComment(text, relPath));
    indexEntries.push({
      slug: entry.slug,
      title: metadata.title || titleFromSlug(entry.slug),
      description: metadata.description || '',
      aliases: parseFrontmatterList(text, 'aliases'),
    });
  }
  ...
  const glossaryIndex = [
    ...
    ...indexEntries.map((entry) => {
      ...
      return `- [${entry.title}](/glossary/${entry.slug}/) -${description}${aliases}`;
    }),
    '',
  ].join('\n');

  await writeFile(websiteGlossaryIndexPath, glossaryIndex);
  return indexEntries;
}
