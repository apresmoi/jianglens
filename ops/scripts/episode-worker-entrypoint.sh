...
LENS_HOME="/var/lib/spawnfile/instances/picoclaw/agent-lens-steward/picoclaw"
LENS_WORKSPACE_DIR="$LENS_HOME/workspace"
...
LENS_REPO_DIR="${LENS_STEWARD_REPO_DIR:-$LENS_WORKSPACE_DIR/jiang-lens}"
MOLTNET_CLIENT_CONFIG_PATH="${MOLTNET_CLIENT_CONFIG:-$WORKSPACE_DIR/.moltnet/config.json}"
LENS_MOLTNET_CLIENT_CONFIG_PATH="${LENS_MOLTNET_CLIENT_CONFIG:-$LENS_WORKSPACE_DIR/.moltnet/config.json}"
PICOCLAW_CONFIG_PATH="${PICOCLAW_CONFIG:-$WORKER_HOME/config.json}"
LENS_PICOCLAW_CONFIG_PATH="${LENS_PICOCLAW_CONFIG:-$LENS_HOME/config.json}"
LOOP_INTERVAL_SECONDS="${EPISODE_WORKER_LOOP_INTERVAL_SECONDS:-86400}"
...
LENS_LOOP_INTERVAL_SECONDS="${LENS_STEWARD_LOOP_INTERVAL_SECONDS:-180}"
LENS_LOOP_SESSION="${LENS_STEWARD_LOOP_SESSION:-agent:lens-steward:autonomous-loop}"
LENS_LOOP_STATE_DIR="${LENS_STEWARD_STATE_DIR:-$LENS_HOME/state/lens-steward}"
LENS_LOOP_LEASE_FILE="$LENS_LOOP_STATE_DIR/lease.json"
LENS_LOOP_HEARTBEAT_FILE="$LENS_LOOP_STATE_DIR/heartbeat.json"
LENS_LOOP_CURRENT_FILE="$LENS_LOOP_STATE_DIR/current.json"
LENS_LOOP_JOURNAL_FILE="$LENS_LOOP_STATE_DIR/journal.md"
LENS_LOOP_FAILURES_FILE="$LENS_LOOP_STATE_DIR/failures.md"
LENS_LOOP_HEARTBEAT_INTERVAL_SECONDS="${LENS_STEWARD_HEARTBEAT_INTERVAL_SECONDS:-45}"
...
mkdir -p "$LENS_HOME"
HOME="$LENS_HOME" \
  CODEX_HOME="$LENS_HOME/.codex" \
  PICOCLAW_HOME="$LENS_HOME" \
  configure-agent-github
...
mkdir -p "$LENS_WORKSPACE_DIR"

if [ ! -d "$LENS_REPO_DIR/.git" ]; then
  echo "Cloning Jiang Lens repo into $LENS_REPO_DIR for lens-steward"
  git clone --branch "$REPO_BRANCH" "$REPO_URL" "$LENS_REPO_DIR"
else
  current_branch="$(git -C "$LENS_REPO_DIR" branch --show-current || true)"
  if [ "$current_branch" = "$REPO_BRANCH" ] && git -C "$LENS_REPO_DIR" diff --quiet && git -C "$LENS_REPO_DIR" diff --cached --quiet; then
    echo "Updating clean Jiang Lens checkout in $LENS_REPO_DIR"
    git -C "$LENS_REPO_DIR" fetch origin "$REPO_BRANCH"
    git -C "$LENS_REPO_DIR" pull --ff-only origin "$REPO_BRANCH"
  else
    echo "Leaving existing Jiang Lens lens-steward checkout unchanged in $LENS_REPO_DIR"
    git -C "$LENS_REPO_DIR" status --short --branch || true
  fi
fi
...
start_lens_steward_loop() {
  if [ "${LENS_STEWARD_LOOP_ENABLED:-true}" = "false" ]; then
    echo "Lens steward loop disabled"
    return
  fi

  (
    set +e
    mkdir -p "$LENS_LOOP_STATE_DIR"
    touch "$LENS_LOOP_JOURNAL_FILE" "$LENS_LOOP_FAILURES_FILE"

    while true; do
      if ! [[ "$LENS_LOOP_INTERVAL_SECONDS" =~ ^[0-9]+$ ]] || [ "$LENS_LOOP_INTERVAL_SECONDS" -lt 30 ]; then
        LENS_LOOP_INTERVAL_SECONDS=180
      fi
      if ! [[ "$LENS_LOOP_HEARTBEAT_INTERVAL_SECONDS" =~ ^[0-9]+$ ]] || [ "$LENS_LOOP_HEARTBEAT_INTERVAL_SECONDS" -lt 10 ]; then
        LENS_LOOP_HEARTBEAT_INTERVAL_SECONDS=45
      fi
      ...
      existing_pid=""
      if [ -f "$LENS_LOOP_LEASE_FILE" ]; then
      ...
      if [ -n "$existing_pid" ] && [ -r "/proc/$existing_pid/cmdline" ]; then
        ...
        case "$existing_cmd" in
          *"picoclaw agent --session $LENS_LOOP_SESSION"*|*"picoclaw agent"*)
            now="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
            cat >"$LENS_LOOP_HEARTBEAT_FILE.tmp" <<JSON
{
  "agent": "lens-steward",
  "status": "running",
  "pid": $existing_pid,
  "session": "$LENS_LOOP_SESSION",
  "updated_at": "$now",
  "supervisor_note": "existing Plato process is still alive"
}
JSON
            mv "$LENS_LOOP_HEARTBEAT_FILE.tmp" "$LENS_LOOP_HEARTBEAT_FILE"
            echo "[lens-steward-loop] previous iteration still running with pid $existing_pid"
            if [ "${LENS_STEWARD_LOOP_ONCE:-false}" = "true" ]; then
              break
            fi
            sleep "$LENS_LOOP_INTERVAL_SECONDS"
            continue
            ;;
        esac
      fi
      ...
      if [ -f "$LENS_LOOP_LEASE_FILE" ]; then
        stale_at="$(date -u +%Y%m%dT%H%M%SZ)"
        mv "$LENS_LOOP_LEASE_FILE" "$LENS_LOOP_STATE_DIR/stale-lease-$stale_at.json"
        echo "[lens-steward-loop] recovered stale Plato lease"
      fi

      started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
      printf '[lens-steward-loop] starting iteration at %s\n' "$started_at"

      prompt="$(cat <<PROMPT
You are Plato, the Jiang Lens lens-steward running from the autonomous worker loop.

This is not a Moltnet auto-reply subprocess. You may run long enough to finish one concrete lens-steward unit of work. You are a Moltnet room participant: use the moltnet skill and the local Moltnet CLI to read local_lab/episode-floor, answer fresh direct mentions, discuss blockers, and post concise status updates. The launcher sets MOLTNET_CLIENT_CONFIG, so ordinary commands like moltnet read --target room:episode-floor --limit 20 and moltnet send --target room:episode-floor --text "..." work after you cd jiang-lens.

You are a durable agent identity, not a disposable cron task. Your persisted runtime state lives at:

- current state: $LENS_LOOP_CURRENT_FILE
- heartbeat: $LENS_LOOP_HEARTBEAT_FILE
- journal: $LENS_LOOP_JOURNAL_FILE
- failures: $LENS_LOOP_FAILURES_FILE

At startup, read those files when they exist. During work, update current state at stage boundaries with the concept/task, branch, stage, and next checkpoint. Use the journal for concise reusable lessons and the failures file for concrete recovery notes. Do not treat memory as authority over source refs, validation, git state, or maintainer instructions.

Loop contract for this iteration:

1. Enter jiang-lens/ inside the Plato Picoclaw workspace and inspect git status --short --branch.
2. Read repo AGENTS.md, then agents/lens-steward/IDENTITY.md, SOUL.md, HEARTBEAT.md, MEMORY.md, and STATE.md.
3. Read recent episode-floor history. The newest maintainer instruction wins. Distinguish fresh maintainer instructions, direct mentions, Virgil handoffs, stale blocker loops, PR closeouts, and background noise.
4. Before heavy work, post one concise reception/state message to episode-floor with current branch, active concept/task if any, and next stage.
5. If the checkout already has a lens branch or uncommitted lens-scoped work, continue that work from the first missing or failing stage. Do not claim a second concept.
6. If the checkout is clean on main, fast-forward from origin/main and choose one meaningful lens mutation from source-backed pressure in the processed corpus.
7. Your ordinary write scope is agents/lens-steward/**, content/workflow/proposals/**, content/workflow/reviews/**, content/workflow/promotions/** only when explicitly promoting reviewed material, website/src/content/docs/lens.md, website/src/content/docs/lens/**, generated outputs from compile scripts, and episode read JSON only when adding or repairing lens_points links.
8. Do not process videos, rewrite episode reads, edit raw sources, edit .codex/skills/**, or change runtime/Spawnfile/ops scripts unless a maintainer explicitly expands scope.
9. Use the Jiang lens distillation skills as methods. A good unit is one concept page mutation, one atlas mutation, one grounded provenance repair, or one compact proposal that supports a specific lens mutation.
10. For substantial public lens changes, run the judge gate. If independent judge agents are unavailable inside this runtime, do the reader/world-model and grounding/provenance passes yourself and say so in the PR notes.
11. Validate before PR:
    node ops/scripts/compile-content.mjs
    node ops/scripts/validate-content.mjs
    cd website && npm run build
    Also run node ops/scripts/validate-corpus-impact.mjs --all when corpus-impact files changed.
12. Open a PR, enable auto-merge only after validation is clean, wait for merge when practical, then return to clean synced main.
13. Exit after one concept/task is merged, blocked, or handed off. The outer supervisor will keep this identity alive and resume from state if the process dies.

Self-improvement rule: you may edit only agents/lens-steward/** for worker-local instructions, memory, or proposals. Do not edit .codex/skills/** directly. If a shared skill, runtime, or process change seems necessary, write a proposal under agents/lens-steward/proposals/ or include it in the PR notes.
PROMPT
)"
      ...
      HOME="$LENS_HOME" \
        CODEX_HOME="$LENS_HOME/.codex" \
        PICOCLAW_HOME="$LENS_HOME" \
        PICOCLAW_CONFIG="$LENS_PICOCLAW_CONFIG_PATH" \
        MOLTNET_CLIENT_CONFIG="$LENS_MOLTNET_CLIENT_CONFIG_PATH" \
        JIANG_LENS_REPO_DIR="$LENS_REPO_DIR" \
        LENS_STEWARD_STATE_DIR="$LENS_LOOP_STATE_DIR" \
        picoclaw agent --session "$LENS_LOOP_SESSION" --message "$prompt" &
      agent_pid="$!"

      cat >"$LENS_LOOP_LEASE_FILE.tmp" <<JSON
{
  "agent": "lens-steward",
  "pid": $agent_pid,
  "session": "$LENS_LOOP_SESSION",
  "started_at": "$started_at",
  "repo_dir": "$LENS_REPO_DIR",
  "state_dir": "$LENS_LOOP_STATE_DIR"
}
JSON
      mv "$LENS_LOOP_LEASE_FILE.tmp" "$LENS_LOOP_LEASE_FILE"
          ...
          cat >"$LENS_LOOP_HEARTBEAT_FILE.tmp" <<JSON
{
  ...
  "started_at": "$started_at",
  "updated_at": "$now",
  "supervisor_note": "Plato process is alive"
}
JSON
          mv "$LENS_LOOP_HEARTBEAT_FILE.tmp" "$LENS_LOOP_HEARTBEAT_FILE"
          sleep "$LENS_LOOP_HEARTBEAT_INTERVAL_SECONDS"
        done
      ) &
      heartbeat_pid="$!"
      ...
      cat >"$LENS_LOOP_HEARTBEAT_FILE.tmp" <<JSON
{
  ...
  "updated_at": "$finished_at",
  "exit_status": $status,
  "supervisor_note": "Plato process exited; next loop may resume from repo and current state"
}
JSON
      mv "$LENS_LOOP_HEARTBEAT_FILE.tmp" "$LENS_LOOP_HEARTBEAT_FILE"
      rm -f "$LENS_LOOP_LEASE_FILE"
      printf '[lens-steward-loop] iteration exited with status %s\n' "$status"
